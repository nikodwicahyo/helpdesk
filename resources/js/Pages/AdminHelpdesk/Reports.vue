<template>
    <AppLayout role="admin">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="today">
                            {{ t("reports.ranges.today") }}
                        </option>
                        <option value="yesterday">
                            {{ t("reports.ranges.yesterday") }}
                        </option>
                        <option value="this_week">
                            {{ t("reports.ranges.this_week") }}
                        </option>
                        <option value="last_week">
                            {{ t("reports.ranges.last_week") }}
                        </option>
                        <option value="this_month">
                            {{ t("reports.ranges.this_month") }}
                        </option>
                        <option value="last_month">
                            {{ t("reports.ranges.last_month") }}
                        </option>
                        <option value="this_quarter">
                            {{ t("reports.ranges.this_quarter") }}
                        </option>
                        <option value="last_quarter">
                            {{ t("reports.ranges.last_quarter") }}
                        </option>
                        <option value="this_year">
                            {{ t("reports.ranges.this_year") }}
                        </option>
                        <option value="last_year">
                            {{ t("reports.ranges.last_year") }}
                        </option>
                    </select>
                </div>
        ...
        >
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="bg-gray-50 rounded-lg p-4">
                        ...
                        <p class="text-2xl font-bold text-gray-900">
                            {{ reportPreview.summary.total_tickets }}
                        </p>
                    </div>
                    <div class="bg-green-50 rounded-lg p-4">
                        <p class="text-sm text-gray-600">Resolved</p>
                        <p class="text-2xl font-bold text-green-600">
                            {{ reportPreview.summary.resolved_tickets }}
                        </p>
                    </div>
                    <div class="bg-blue-50 rounded-lg p-4">
                        <p class="text-sm text-gray-600">In Progress</p>
                        <p class="text-2xl font-bold text-blue-600">
                            {{ reportPreview.summary.in_progress_tickets }}
                        </p>
                    </div>
                    <div class="bg-yellow-50 rounded-lg p-4">
                        <p class="text-sm text-gray-600">Avg Resolution Time</p>
                        <p class="text-2xl font-bold text-yellow-600">
                            {{ reportPreview.summary.avg_resolution_time }}h
                        </p>
                    </div>
                </div>
            </div>
            ...
            <!-- Charts Section -->
            <div
                v-if="includeOptions.charts && reportPreview.charts"
                class="p-6 border-b border-gray-200"
            >
                <h3 class="text-lg font-semibold text-gray-900 mb-4">
                    Visual Analytics
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr
                                v-for="teknisi in reportPreview.performance"
                                :key="teknisi.nip"
            ...
            <div
                v-if="
                    selectedReportType === 'sla' && reportPreview.sla_compliance
                "
                class="p-6 border-b border-gray-200"
            >
                <h3 class="text-lg font-semibold text-gray-900 mb-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div class="bg-green-50 rounded-lg p-4">
                        ...
                        <p class="text-2xl font-bold text-green-600">
                            {{ reportPreview.sla_compliance.within_sla }}
                        </p>
                    </div>
                    <div class="bg-red-50 rounded-lg p-4">
                        <p class="text-sm text-gray-600">SLA Breached</p>
                        <p class="text-2xl font-bold text-red-600">
                            {{ reportPreview.sla_compliance.sla_breached }}
                        </p>
                    </div>
                    <div class="bg-blue-50 rounded-lg p-4">
                        <p class="text-sm text-gray-600">Compliance Rate</p>
                        <p class="text-2xl font-bold text-blue-600">
                            {{ reportPreview.sla_compliance.compliance_rate }}%
                        </p>
                    </div>
                </div>
                ...
                <!-- SLA by Priority -->
                <div
                    v-if="reportPreview.sla_compliance.priority_breakdown"
                    class="mt-6"
                >
                    <h4 class="text-md font-medium text-gray-900 mb-3">
                        SLA by Priority
                    ...
                    >
                        <div
                            v-for="(data, priority) in reportPreview
                                .sla_compliance.priority_breakdown"
                            :key="priority"
                        ...
                        >
                            ...
                            <p class="text-sm text-green-600">
                                Within SLA: {{ data.within_sla }}
                            </p>
                            <p class="text-sm text-red-600">
                                SLA Breached: {{ data.sla_breached }}
                            </p>
            ...
            <div
                v-if="
                    ...
                    reportPreview.application_breakdown
                "
                class="p-6 border-b border-gray-200"
            >
                <h3 class="text-lg font-semibold text-gray-900 mb-4">
                ...
                >
                    <div
                        v-for="app in reportPreview.application_breakdown"
                        :key="app.aplikasi_id"
                        class="bg-gray-50 rounded-lg p-4"
                    >
                ...
                <div class="mb-6">
                    <p class="text-sm text-gray-600">
                        ...
                        <span class="font-bold text-gray-900">{{
                            reportPreview.user_activity.total_users_active
                        }}</span>
                    </p>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr
                                v-for="user in reportPreview.user_activity
                                    .user_activity"
                                :key="user.user?.nama_lengkap"
                                class="hover:bg-gray-50"
                            >
                                <td
                                    class="px-6 py-4 text-sm font-medium text-gray-900"
                                >
                                    {{ user.user?.nama_lengkap || "Unknown" }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    {{ user.tickets_created }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    {{ user.tickets_resolved }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-900">
            ...
            >
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    <div class="bg-blue-50 rounded-lg p-4">
                        ...
                        <p class="text-2xl font-bold text-blue-600">
                            {{
                                reportPreview.executive_summary.overview
                                    .sla_compliance_rate
                            }}%
                        </p>
                    </div>
                ...
                <!-- Top Applications -->
                <div
                    v-if="reportPreview.executive_summary.top_applications"
                    class="mb-6"
                >
                    <h4 class="text-md font-medium text-gray-900 mb-3">
                        Top Applications
                    ...
                    >
                        <div
                            v-for="app in reportPreview.executive_summary
                                .top_applications"
                            :key="app.aplikasi"
                            class="bg-gray-50 rounded-lg p-4"
                        >
                ...
                <!-- Top Teknisi -->
                <div
                    v-if="reportPreview.executive_summary.top_teknisi"
                    class="mb-6"
                >
                    <h4 class="text-md font-medium text-gray-900 mb-3">
                        Top Performing Teknisi
                    </h4>
                    <div
                        class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4"
                    >
                        <div
                            v-for="teknisi in reportPreview.executive_summary
                                .top_teknisi"
                            :key="teknisi.nip"
                            class="bg-gray-50 rounded-lg p-4"
                        >
                            ...
                            <p class="text-sm text-gray-600">
                                Resolved: {{ teknisi.resolved_tickets }}
                            </p>
            ...
            <div v-if="includeOptions.details" class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr
                            v-for="ticket in reportPreview.data"
                            :key="ticket.id"
                            class="hover:bg-gray-50"
                        >
                            <td
                                class="px-6 py-4 text-sm font-mono text-gray-900"
                            >
                        ...
                        >
                            ...
                            >
                                <form @submit.prevent="confirmSchedule">
                                    ...
                                    >
                                        ...
                                        >
                                            <div class="flex items-start">
                                                <div
                                                    class="flex-shrink-0 flex items-center justify-center h-10 w-10 rounded-full bg-purple-100"
                                                >
                                                    <svg
                                    ...
                                    <!-- Actions -->
                                    <div
                                        class="bg-gray-50 px-6 py-4 border-t border-gray-200 flex flex-row-reverse gap-3"
                                    >
                                        <button
                                            type="submit"
                                            :disabled="scheduling"
...
const reportTypes = [
    {
        key: "tickets",
        label: t("reports.types.tickets.label"),
        description: t("reports.types.tickets.description"),
        color: "bg-blue-500",
    },
    {
        key: "performance",
        label: t("reports.types.performance.label"),
        description: t("reports.types.performance.description"),
        color: "bg-green-500",
    },
    {
        key: "user_activity",
        label: t("reports.types.user_activity.label"),
        description: t("reports.types.user_activity.description"),
        color: "bg-purple-500",
    },
    {
        key: "sla",
        label: t("reports.types.sla.label"),
        description: t("reports.types.sla.description"),
        color: "bg-orange-500",
    },
    {
        key: "application",
        label: t("reports.types.application.label"),
        description: t("reports.types.application.description"),
        color: "bg-indigo-500",
    },
    {
        key: "summary",
        label: t("reports.types.summary.label"),
        description: t("reports.types.summary.description"),
        color: "bg-red-500",
    },
];
...
const canGenerateReport = computed(() => {
    return (
        selectedReportType.value && dateRange.value.from && dateRange.value.to
    );
});
...
const exportReport = async (format) => {
    try {
        ...
        const response = await fetch(
            route("admin.reports.export") + "?" + params.toString(),
            {
                method: "GET",
                headers: {
                    Accept:
...
const scheduleReport = () => {
    ...
    scheduleData.value.title = `${selectedReportType.value.replace(
        "_",
        " "
    )} report`;
    ...
    try {
        ...
        if (editingScheduleId.value) {
            ...
            response = await fetch(
                route(
                    "admin.reports.scheduled-reports.update",
            ...
            // Create new schedule
            response = await fetch(
                route("admin.reports.scheduled-reports.store"),
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
...
const deleteScheduledReport = (report) => {
    if (
        confirm(
            `Are you sure you want to delete the scheduled report "${report.name}"?`
        )
    ) {
