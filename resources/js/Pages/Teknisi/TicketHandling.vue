<template>
    <AppLayout role="teknisi">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 flex items-center">
                        ...
                        <span v-if="stats.urgent_tickets > 0" class="ml-3 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-800 animate-pulse">
                            {{ stats.urgent_tickets }} Urgent
                        </span>
                    </h1>
                    <p class="text-gray-500 mt-1 text-sm sm:text-base">Manage and resolve your assigned tickets efficiently</p>
                </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-5 hover:shadow-md transition-shadow duration-200">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide">Assigned</p>
                        <p class="mt-1 text-2xl sm:text-3xl font-bold text-gray-900">{{ stats.assigned_tickets }}</p>
                        <p class="mt-1 text-xs text-gray-400">Currently assigned</p>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-5 hover:shadow-md transition-shadow duration-200">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide">In Progress</p>
                        <p class="mt-1 text-2xl sm:text-3xl font-bold text-indigo-600">{{ stats.in_progress_tickets }}</p>
                        <p class="mt-1 text-xs text-gray-400">Being worked on</p>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-5 hover:shadow-md transition-shadow duration-200">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide">Resolved Today</p>
                        <p class="mt-1 text-2xl sm:text-3xl font-bold text-green-600">{{ stats.resolved_today }}</p>
                        <p v-if="stats.resolved_today_trend > 0" class="mt-1 text-xs text-green-500 flex items-center">
                        ...
                        <p v-else class="mt-1 text-xs text-gray-400">Great work!</p>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-5 hover:shadow-md transition-shadow duration-200">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide">Avg Resolution</p>
                        <p class="mt-1 text-2xl sm:text-3xl font-bold text-amber-600">{{ stats.avg_resolution_time }}h</p>
                        <p class="mt-1 text-xs text-gray-400">This week</p>
                    </div>
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                ...
                <div class="flex items-center justify-between sm:justify-end gap-4">
                    <div class="flex items-center text-sm text-gray-500">
                        <span class="font-medium text-gray-900">{{ filteredTickets.length }}</span>
                        <span class="ml-1">ticket{{ filteredTickets.length !== 1 ? 's' : '' }}</span>
                    </div>
        ...
        <!-- Kanban Board View -->
        <div v-if="viewMode === 'kanban'" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6">
            <!-- Assigned Column -->
            <div class="bg-gradient-to-b from-amber-50 to-white rounded-xl border border-amber-100 shadow-sm overflow-hidden">
                <div class="p-4 bg-amber-50/80 border-b border-amber-100">
                    <div class="flex items-center justify-between">
                        <h3 class="font-semibold text-gray-900 flex items-center text-sm">
                            <span class="w-2.5 h-2.5 bg-amber-500 rounded-full mr-2 shadow-sm"></span>
                            Assigned
                        </h3>
                ...
                <div
                    :class="[
                        ...
                        isDragging && dragTarget === 'assigned' ? 'bg-amber-100/50' : ''
                    ]"
                    @dragover.prevent="dragTarget = 'assigned'"
                    @dragleave="dragTarget = null"
                    @drop="handleDrop($event, 'assigned')"
                >
                    <TransitionGroup name="ticket-list" tag="div" class="space-y-3">
                        <KanbanTicket
                    ...
                    <div v-if="getColumnTickets('assigned').length === 0" class="flex flex-col items-center justify-center text-gray-400 py-12">
                        <div class="p-3 bg-gray-100 rounded-full mb-3">
                            <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
                            </svg>
                        </div>
                        <p class="text-sm font-medium">No assigned tickets</p>
                        <p class="text-xs mt-1">New tickets will appear here</p>
                    </div>
                </div>
            </div>

            <!-- In Progress Column -->
            <div class="bg-gradient-to-b from-blue-50 to-white rounded-xl border border-blue-100 shadow-sm overflow-hidden">
                <div class="p-4 bg-blue-50/80 border-b border-blue-100">
                    <div class="flex items-center justify-between">
                        <h3 class="font-semibold text-gray-900 flex items-center text-sm">
                            <span class="w-2.5 h-2.5 bg-blue-500 rounded-full mr-2 shadow-sm animate-pulse"></span>
                            In Progress
                        </h3>
                ...
                <div
                    :class="[
                        'p-3 min-h-[350px] space-y-3 transition-colors duration-200',
                        isDragging && dragTarget === 'in_progress' ? 'bg-blue-100/50' : ''
                    ]"
                ...
                >
                    ...
                    <div v-if="getColumnTickets('in_progress').length === 0" class="flex flex-col items-center justify-center text-gray-400 py-12">
                        <div class="p-3 bg-gray-100 rounded-full mb-3">
                            <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M13 10V3L4 14h7v7l9-11h-7z" />
                            </svg>
                        </div>
                        <p class="text-sm font-medium">No tickets in progress</p>
                        <p class="text-xs mt-1">Drag tickets here to start</p>
                    </div>
                </div>
            </div>

            <!-- Waiting Response Column -->
            <div class="bg-gradient-to-b from-orange-50 to-white rounded-xl border border-orange-100 shadow-sm overflow-hidden">
                <div class="p-4 bg-orange-50/80 border-b border-orange-100">
                    <div class="flex items-center justify-between">
                        <h3 class="font-semibold text-gray-900 flex items-center text-sm">
                            <span class="w-2.5 h-2.5 bg-orange-500 rounded-full mr-2 shadow-sm"></span>
                            Waiting Response
                        </h3>
                        <span class="bg-orange-100 text-orange-700 px-2.5 py-0.5 rounded-full text-xs font-semibold">
                ...
                <div
                    :class="[
                        'p-3 min-h-[350px] space-y-3 transition-colors duration-200',
                        isDragging && dragTarget === 'waiting_response' ? 'bg-orange-100/50' : ''
                    ]"
                ...
                >
                    <TransitionGroup name="ticket-list" tag="div" class="space-y-3">
                        <KanbanTicket
                            v-for="ticket in getColumnTickets('waiting_response')"
                            :key="ticket.id"
                            :ticket="ticket"
                            :draggable="true"
                            @dragstart="handleDragStart($event, ticket)"
                    ...
                    <div v-if="getColumnTickets('waiting_response').length === 0" class="flex flex-col items-center justify-center text-gray-400 py-12">
                        <div class="p-3 bg-gray-100 rounded-full mb-3">
                            <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                        </div>
                        <p class="text-sm font-medium">No pending tickets</p>
                        <p class="text-xs mt-1">Waiting for user response</p>
                    </div>
                </div>
            </div>

            <!-- Resolved Column -->
            <div class="bg-gradient-to-b from-green-50 to-white rounded-xl border border-green-100 shadow-sm overflow-hidden">
                <div class="p-4 bg-green-50/80 border-b border-green-100">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="bg-green-100 text-green-700 px-2.5 py-0.5 rounded-full text-xs font-semibold">
                ...
                <div
                    :class="[
                        'p-3 min-h-[350px] space-y-3 transition-colors duration-200',
                        isDragging && dragTarget === 'resolved' ? 'bg-green-100/50' : ''
                    ]"
                ...
                >
                    ...
                    <div v-if="getColumnTickets('resolved').length === 0" class="flex flex-col items-center justify-center text-gray-400 py-12">
                        <div class="p-3 bg-gray-100 rounded-full mb-3">
                            <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                        </div>
                        <p class="text-sm font-medium">No resolved tickets</p>
                        <p class="text-xs mt-1">Completed tickets appear here</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        >
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm">
                                    <p class="font-medium text-gray-900">{{ ticket.user?.nama_lengkap }}</p>
                                    <p class="text-gray-500">{{ ticket.user?.email }}</p>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <select
                                ...
                                >
                                    <option value="assigned" v-if="ticket.status === 'open' || ticket.status === 'assigned'" disabled>Assigned (Start Working →)</option>
                                    <option value="in_progress">In Progress</option>
                                    <option value="waiting_user">Waiting Response</option>
                                    <option value="resolved">Resolved</option>
                                </select>
                            </td>
                            <td class="px-6 py-4">
    ...
    if (priorityFilter.value) {
        switch (priorityFilter.value) {
            case 'urgent':
                ...
                break;
            case 'high':
                filtered = filtered.filter(ticket => ['urgent', 'high'].includes(ticket.priority));
                break;
            case 'overdue':
    ...
    if (statusFilter.value) {
        switch (statusFilter.value) {
            ...
            case 'waiting_response':
                filtered = filtered.filter(ticket => ticket.status === 'waiting_response' || ticket.status === 'pending');
                break;
            case 'resolved':
...
const getColumnTickets = (status) => {
    switch (status) {
        ...
        case 'waiting_response':
            ...
            return filteredTickets.value.filter(ticket => 
                ticket.status === 'waiting_user' || ticket.status === 'waiting_response'
            );
        default:
...
const handleStatusChange = (ticket, newStatus) => {
    ...
    if (newStatus === 'assigned') {
        alert('Cannot change status back to Assigned. Use the ticket detail page to manage this ticket.');
        return;
    }
...
const handleDrop = (event, newStatus) => {
    ...
    if (draggedTicket.value) {
        ...
        if (newStatus === 'assigned' && currentStatus !== 'assigned') {
            alert('Cannot move ticket back to Assigned. Tickets can only move forward in the workflow.');
...
const updateTicketStatus = async (ticketId, newStatus) => {
    ...
    try {
        ...
        if (error.response?.data?.errors) {
            ...
            errorMsg = `Status '${error.response.data.debug.requested_status}' not allowed. Allowed: ${error.response.data.debug.allowed_statuses.join(', ')}`;
        }
...
const markAsClosed = async (ticketId) => {
    ...
    alert('Tickets can only be closed by the user who created them after reviewing the resolution.');
};
