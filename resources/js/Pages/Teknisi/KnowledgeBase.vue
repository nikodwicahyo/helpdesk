<template>
    <AppLayout role="teknisi">
            ...
            <div class="flex items-center justify-between w-full">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-gray-500 text-sm">Documentation and solutions library</p>
                    </div>
                </div>
                <div class="flex items-center space-x-3">
                    <button
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-8">
            ...
            <div class="bg-white rounded-xl p-5 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100 group relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <h3 class="text-xs font-medium text-gray-500 uppercase tracking-wide">Total Articles</h3>
                    <div class="mt-1 flex items-baseline">
                        <p class="text-3xl font-bold text-gray-900">{{ stats.total_articles || 0 }}</p>
                    </div>
                    <p class="mt-1 text-xs text-gray-500">in knowledge base</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl p-5 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100 group relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <h3 class="text-xs font-medium text-gray-500 uppercase tracking-wide">This Month</h3>
                    <div class="mt-1 flex items-baseline">
                        <p class="text-3xl font-bold text-gray-900">{{ stats.articles_this_month || 0 }}</p>
                    </div>
                    <p class="mt-1 text-xs text-gray-500">new articles</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl p-5 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100 group relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <h3 class="text-xs font-medium text-gray-500 uppercase tracking-wide">My Published</h3>
                    <div class="mt-1 flex items-baseline">
                        <p class="text-3xl font-bold text-gray-900">{{ stats.my_published || 0 }}</p>
                    </div>
            ...
            <div class="bg-white rounded-xl p-5 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100 group relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <h3 class="text-xs font-medium text-gray-500 uppercase tracking-wide">My Drafts</h3>
                    <div class="mt-1 flex items-baseline">
                        <p class="text-3xl font-bold text-gray-900">{{ stats.my_drafts || 0 }}</p>
                    </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-6 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Search Knowledge Base</label>
                    <div class="relative">
                        <input
            ...
            <div class="mt-4 pt-4 border-t border-gray-200">
                <div class="flex items-center justify-between mb-3">
                    <div class="flex items-center space-x-4">
                        ...
                        <label class="inline-flex items-center cursor-pointer">
                            ...
                            <span class="ms-3 text-sm font-medium text-gray-700">My Articles Only</span>
                        </label>
                        <button
                ...
                <div v-show="showAdvancedFilters" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <!-- Application Filter -->
                    <div>
        ...
        <!-- Popular Tags -->
        <div v-if="popularTags.length > 0" class="bg-white rounded-xl shadow-lg border border-gray-100 p-6 mb-6">
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center space-x-3">
                    ...
                    <h3 class="text-base font-semibold text-gray-900">Popular Tags</h3>
                </div>
                <span v-if="selectedTags.length > 0" class="text-xs text-indigo-600 font-medium">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                >
                    ...
                    <span :class="[
                        ...
                        selectedTags.includes(tag.name) ? 'bg-white/20' : 'bg-gray-200'
                    ]">{{ tag.count }}</span>
                </button>
            </div>
            <div v-if="selectedTags.length > 0" class="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between">
                <span class="text-sm text-gray-500">Filtering by {{ selectedTags.length }} tag(s)</span>
                <button
                    @click="selectedTags = []; applyFilters()"
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <!-- Empty State -->
            <div v-else-if="!articles.data || articles.data.length === 0" class="p-12 text-center">
                <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                </svg>
                <h3 class="mt-2 text-sm font-medium text-gray-900">No articles found</h3>
                <p class="mt-1 text-sm text-gray-500">
                    {{ hasActiveFilters ? 'Try adjusting your filters to find more knowledge base articles' : 'No resolved tickets with solutions available yet' }}
                </p>
                    ...
                    >
                        ...
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center space-x-2">
                                ...
                                <span :class="['px-2 py-1 text-xs font-medium rounded-full', getStatusColor(article.status)]">
                                    {{ article.status }}
                                </span>
                            </div>
                            <div class="flex items-center space-x-1">
                                <span class="text-xs text-gray-500">👁️ {{ article.views || 0 }}</span>
                                <span v-if="article.helpful_count > 0" class="text-xs text-green-600">👍 {{ article.helpful_count }}</span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2 mb-4">
                            <div v-if="article.kategori_masalah" class="flex items-center text-sm text-gray-500">
                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                                </svg>
                                {{ article.kategori_masalah.name }}
                            </div>
                            <div v-if="article.aplikasi" class="flex items-center text-sm text-gray-500">
                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-gray-200">
                            ...
                            <div v-if="article.can_edit" class="flex items-center space-x-2">
                                <button
                        ...
                        >
                            <td class="px-6 py-4">
                                <div>
                                    ...
                                    <p v-if="article.excerpt" class="text-xs text-gray-500 mt-1">{{ article.excerpt }}</p>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span v-if="article.kategori_masalah" class="text-sm text-gray-900">{{ article.kategori_masalah.name }}</span>
...
const exportKnowledgeBase = () => {
    ...
    window.open(route('teknisi.knowledge-base.export-all') + '?' + params.toString(), '_blank');
};
...
onMounted(() => {
    ...
    if (props.filters.tags) {
        selectedTags.value = Array.isArray(props.filters.tags) ? props.filters.tags : props.filters.tags.split(',');
    }
});
</script>
