                                ...
                                <div class="space-y-4">
                                    ...
                                    <div>
                                        <label for="default_priority" class="block text-sm font-medium text-gray-700">
                                            {{ t('modal.categoryModal.defaultPriority') }}
                                        </label>
                                        <select
                                            id="default_priority"
                                        ...
                                        <div class="bg-gray-50 p-4 rounded-lg">
                                            ...
                                            <dl class="grid grid-cols-2 gap-2 text-sm">
                                                <div>
                                                    ...
                                                    <dd class="font-medium text-gray-900">{{ category.ticket_count || 0 }}</dd>
                                                </div>
                                                <div>
...
watch(() => props.category, (cat) => {
    if (cat && props.mode === 'edit') {
        ...
        form.default_priority = cat.default_priority || 'medium';
        form.clearErrors();
    } else if (props.mode === 'create') {
        form.reset();
        form.status = 'active';
        form.default_priority = 'medium';
        if (props.applicationId) {
            form.aplikasi_id = props.applicationId;
        }
    }
}, { immediate: true });

// Also watch applicationId prop
watch(() => props.applicationId, (appId) => {
    if (appId && props.mode === 'create') {
        form.aplikasi_id = appId;
    }
}, { immediate: true });
...
const submit = () => {
    ...
    const url = props.mode === 'create'
        ? route('admin-aplikasi.categories.store')
        : route('admin-aplikasi.categories.update', props.category.id);
